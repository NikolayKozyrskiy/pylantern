...
from pylantern.tasks.gan.pix2pix.output_dispatchers import FaceGFPGANOutputDispatcher
...
if TYPE_CHECKING:
    from pylantern.tasks.gan.pix2pix.configs import FaceGFPGANConfig

warnings.filterwarnings("ignore", module="torch.optim.lr_scheduler")
warnings.simplefilter("ignore")


def train_face_gfpgan_fn(
    loop: Loop,
    config_path: Path,
    config_cls: Type[FaceGFPGANConfig],
) -> None:
    config: "FaceGFPGANConfig" = load_config(config_path, config_cls)
    seed_everything(42)
    ...
    pipeline: "FaceGFPGANPipeline" = face_gfpgan_pipeline_from_config(config, device)
    config.preprocess(loop, pipeline)

    out_dispatcher = FaceGFPGANOutputDispatcher(config=config, device=device)
        ...
        for epoch in loop.iterate_epochs(config.max_epoch):
            ...
            for iter_idx, batch in enumerate(
                ...
                with pipeline.batch_scope(batch):
                    ...
                    set_requires_grad(
                        module=pipeline.discriminator_left_eye, value=False
                    )
                    set_requires_grad(
                        module=pipeline.discriminator_right_eye, value=False
                    )
                    set_requires_grad(module=pipeline.discriminator_mouth, value=False)
                    ...
                    # Optimize discriminator_left_eye
                    if out_dispatcher.has_group("component_left_eye"):
                        set_requires_grad(
                            module=pipeline.discriminator_left_eye, value=True
                        )
                        ...
                        scheduler_step_batch(
                            scheduler=scheduler_discriminator_left_eye, num=iter_idx
                        )
                        log_optimizer_single_group_lr(
                            loop,
                    ...
                    # Optimize discriminator_right_eye
                    if out_dispatcher.has_group("component_right_eye"):
                        set_requires_grad(
                            module=pipeline.discriminator_right_eye, value=True
                        )
                        ...
                        scheduler_step_batch(
                            scheduler=scheduler_discriminator_right_eye, num=iter_idx
                        )
                        log_optimizer_single_group_lr(
                            loop,
                    ...
                    # Optimize discriminator_mouth
                    if out_dispatcher.has_group("component_mouth"):
                        set_requires_grad(
                            module=pipeline.discriminator_mouth, value=True
                        )
                        ...
                        scheduler_step_batch(
                            scheduler=scheduler_discriminator_mouth, num=iter_idx
                        )
                        log_optimizer_single_group_lr(
                            loop,
            ...
            scheduler_step_epoch(scheduler=scheduler_discriminator_left_eye, num=epoch)
            scheduler_step_epoch(scheduler=scheduler_discriminator_right_eye, num=epoch)
            scheduler_step_epoch(scheduler=scheduler_discriminator_mouth, num=epoch)
