...
from pylantern.tasks.gan.pix2pix.output_dispatchers import FaceSpadeOutputDispatcher
...
if TYPE_CHECKING:
    from pylantern.tasks.gan.pix2pix.configs import FaceSpadeConfig

warnings.filterwarnings("ignore", module="torch.optim.lr_scheduler")
warnings.simplefilter("ignore")


def train_face_spade_fn(
    loop: Loop,
    config_path: Path,
    config_cls: Type["FaceSpadeConfig"],
) -> None:
    config: "FaceSpadeConfig" = load_config(config_path, config_cls)
    seed_everything(42)
    ...
    pipeline: "FaceSpadePipeline" = face_spade_pipeline_from_config(config, device)
    ...
    out_dispatcher = FaceSpadeOutputDispatcher(config=config, device=device)
