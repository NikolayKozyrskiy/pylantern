...
def p2p_discriminator(
    ...
    if norm == "spectral":
        netD = MultiscaleDiscriminatorSN(
            input_nc,
            ndf,
            n_layers_D,
            use_sigmoid,
...
class LocalEnhancer(nn.Module):
    def __init__(
        ...
        for n in range(1, n_local_enhancers + 1):
            ...
            # model_upsample += [nn.Upsample(scale_factor = 2, mode='bilinear'),
...
class Encoder(nn.Module):
    ...
    def forward(self, input: Tensor, inst) -> Tensor:
...
class MultiscaleDiscriminator(nn.Module):
    ...
    def singleD_forward(self, models: List[nn.Module], input: Tensor) -> List[Tensor]:
        if self.getIntermFeat:
            result = [input]
            for i in range(len(models)):
                result.append(models[i](result[-1]))
            return result[1:]
        else:
            return [models(input)]
...
class MultiscaleDiscriminatorSN(nn.Module):
    def __init__(
        self,
        input_nc: int,
        ndf: int = 64,
        ...
        super(MultiscaleDiscriminatorSN, self).__init__()
        self.num_D = num_D
        self.n_layers = n_layers
        self.getIntermFeat = getIntermFeat

        for i in range(num_D):
            netD = NLayerDiscriminatorSN(
                input_nc, ndf, n_layers, use_sigmoid, getIntermFeat
            )
            if getIntermFeat:
                for j in range(n_layers + 2):
                    setattr(
...
# Defines the PatchGAN discriminator with spectral norm
