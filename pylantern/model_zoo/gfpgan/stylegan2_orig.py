...
def make_resample_kernel(k: List[int]) -> Tensor:
...
class UpFirDnUpsample(nn.Module):
    ...
    def __init__(self, resample_kernel: List[int], factor: int = 2):
...
class ModulatedConv2d(nn.Module):
    ...
    def __init__(
        ...
        resample_kernel: List[int] = (1, 3, 3, 1),
        eps: float = 1e-8,
    ):
...
class ToRGB(nn.Module):
    ...
    def __init__(
        ...
        out_channels = 4 if predict_mask else 3
        if upsample:
...
class StyleGAN2Generator(nn.Module):
    ...
    def mean_latent(self, num_latent: int) -> Tensor:
